  ...
  const toggleFavorite = async (locationName) => {
    try {
      if (favorites.includes(locationName)) {
        ...
        await axios.delete(`${BACKEND_API_URL}/favorites/${encodeURIComponent(locationName)}`);
        setFavorites(favorites.filter(fav => fav !== locationName));
      } else {
        // Add to favorites
        await axios.post(`${BACKEND_API_URL}/favorites`, { location: locationName });
    ...
    <div className="container">
      <div className="weather-app">
        <SearchLocation 
          ...
          isFavorite={weatherData && favorites.includes(weatherData.location.name)}
