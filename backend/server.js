...
app.get("/api/favorites", (req, res) => {
  ...
  db.all(
    "SELECT location FROM favorites ORDER BY created_at DESC",
    [],
    (err, rows) => {
      if (err) {
...
app.post("/api/favorites", (req, res) => {
  ...
  db.run(
    "INSERT INTO favorites (location) VALUES (?)",
    [location],
    function (err) {
      if (err) {
      ...
      console.log(
        `Added location to favorites: ${location} with ID ${this.lastID}`
      );
      res
        .status(201)
        .json({ message: "Location added to favorites", location });
    }
  );
});
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0) {
        ...
        return res
          .status(404)
          .json({ error: "Location not found in favorites" });
      }
